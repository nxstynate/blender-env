...
class PieSnapping(Menu):
    ...
    def draw(self, context):
        ...
        if absolute_grid or (absolute_grid and volume):
            ...
            if bpy.app.version >= (4, 2, 0):
                depress = ts.snap_elements == {'GRID'}
            else:
                depress = ts.snap_elements == {'INCREMENT'} and ts.use_snap_grid_absolute
...
class PieTools(Menu):
    ...
    def draw(self, context):
        ...
        global hypercursor, hardops, boxcutter, hops_names, bc_name 
        ...
        show_hardops = hardops and get_prefs().tools_show_hardops and any(name in tools for name in hops_names)
        show_boxcutter = boxcutter and get_prefs().tools_show_boxcutter and bc_name in tools
        show_hardops_menu = show_hardops and get_prefs().tools_show_hardops_menu and not is_annotate
        ...
        if context.mode in ['OBJECT', 'EDIT_MESH']:
            ...
            if show_hardops:
                tool = tools[hops_names[0 if context.mode == 'OBJECT' else 1]]
                pie.operator("machin3.set_tool_by_name", text="   " + tool['label'], depress=tool['active'], icon_value=tool['icon_value']).name = hops_names[0 if context.mode == 'OBJECT' else 1]
            else:
                pie.separator()
            ...
            if is_annotate:
                ...
                if active_tool in ['builtin.annotate', 'builtin.annotate_line']:
                    row.prop(ts, 'annotation_stroke_placement_view3d', expand=True)
                ...
                if annotate_layer is None:
                    ...
                    column.operator("machin3.toggle_annotation", text=f"{action} Annotations", icon=icon)
