...
if bpy.app.version >= (4, 0, 0):
    has_settings.append('OT_filebrowser_adjust_thumbnail_size')
...
class MACHIN3toolsPreferences(bpy.types.AddonPreferences):
    ...
    filebrowser_show: BoolProperty(name="Show FileBrowser Preferences", default=False)
    filebrowser_modal_thumbnail: BoolProperty(name="Modal Thumbnail Size Adjustment", description="", default=True)
    ...
    def draw_general(self, layout):
        ...
        if bpy.app.version >= (4, 0, 0) and getattr(bpy.types, "MACHIN3_OT_filebrowser_adjust_thumbnail_size", False):
            bb = b.box()
            bb.prop(self, 'filebrowser_show', text="Filebrowser", icon='TRIA_DOWN' if self.filebrowser_show else 'TRIA_RIGHT', emboss=False)

            if self.filebrowser_show:
                column = bb.column(align=True)

                draw_split_row(self, column, prop='filebrowser_modal_thumbnail', label="Modal Thumbnail Size Adjustment")
        ...
        if getattr(bpy.types, "MACHIN3_MT_tools_pie", False):
            ...
            if self.tools_pie_show:
                ...
                if boxcutter:
                    ...
                    row = draw_split_row(self, r, prop='tools_show_hardops_menu', label='Show BoxCutter Presets', factor=0.4)
                    draw_empty_split_row(self, row, factor=0.4)
        ...
        for name in keysdict:
            if "PIE" not in name:
                ...
                if name == 'FILEBROWSER':
                    ...
                    if bpy.app.version >= (4, 0, 0) and self.filebrowser_modal_thumbnail:
                        keylist.pop(5)
                    
                    else:
                        keylist[4]['label'] = "Increase Size"
                        keylist[5]['label'] = "Decrease Size"
