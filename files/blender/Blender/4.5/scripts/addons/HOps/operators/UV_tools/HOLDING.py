...
class HOPS_OT_Draw_UV_Launcher(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.use_selected_meshes == True:
            set_uv_draw_data_multi(context, self.use_selected_faces, self.use_tagged_faces, self.show_all_and_highlight_sel)
        # -----------------------------------
        ...
        # Single draw data
            ...
            if self.target_obj in bpy.data.objects.keys():
                obj = bpy.data.objects[self.target_obj]
                set_uv_draw_data_single(obj, self.use_selected_faces, self.use_tagged_faces, self.show_all_and_highlight_sel)
...
def set_uv_draw_data_single(obj, use_selected_faces=False, use_tagged_faces=False, show_all_and_highlight_sel=False):
    '''Get a list of list vector 2 that represents the uv edges.  [[Vec2, Vec2, Vec2], ... ]'''
    ...
    if show_all_and_highlight_sel:
        for mesh in meshes:
            container = highlight_layer(mesh)
            if container != None:
                Data.containers.append(container)
...
def set_uv_draw_data_multi(context, use_selected_faces=False, use_tagged_faces=False, show_all_and_highlight_sel=False):
    '''Get a list of list vector 2 that represents the uv edges.  [[Vec2, Vec2, Vec2], ... ]'''
