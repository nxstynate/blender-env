...
class Window():
    ...
    def __init__(self, db=None, window_key=""):
        ...
        # Checks
        self.has_setup_factory_load = False
    ...
    def setup(self):
        ...
        elif self.window_key == "Video_Page":
            self.bottom_left = self.db.prefs.ui.Hops_modal_video_window_bottom_left
            self.scale = self.db.prefs.ui.Hops_modal_video_window_scale

        elif self.window_key == "Every_Scroll":
            self.bottom_left = self.db.prefs.ui.Hops_modal_every_scroll_window_bottom_left
            self.scale = self.db.prefs.ui.Hops_modal_every_scroll_window_scale

        # Factory load
        if self.has_setup_factory_load == False:
            self.check_initial_window_placement()
            self.has_setup_factory_load = True
        ...
        if self.can_stack_vertical == True:
        ...
        if self.window_key == "Main":
            ...
            if self.bottom_left[0] == 20 and self.bottom_left[1] == 30:
                ...
                self.bottom_left = (
                    (screen_width * .5) - (win_width * .5),
                    30 * self.db.scale_factor)
                self.db.prefs.ui.Hops_modal_video_window_bottom_left = self.bottom_left
                self.win_gizmo.bottom_left = self.bottom_left

        elif self.window_key == "Every_Scroll":
            # The defualt factory load location
            if self.bottom_left[0] == 20 and self.bottom_left[1] == 30:
                self.bottom_left = (100 * self.db.scale_factor, 80 * self.db.scale_factor)
                self.db.prefs.ui.Hops_modal_every_scroll_window_bottom_left = self.bottom_left
                self.db.prefs.ui.Hops_modal_every_scroll_window_scale = (220, 300)
                self.win_gizmo.bottom_left = self.bottom_left
...
class Window_Gizmo():
    ...
    def setup(self):
        ...
        elif self.window.window_key == "Video_Page":
            self.bottom_left = self.window.db.prefs.ui.Hops_modal_video_window_bottom_left
            self.scale = self.window.db.prefs.ui.Hops_modal_video_window_scale

        elif self.window.window_key == "Every_Scroll":
            self.bottom_left = self.window.db.prefs.ui.Hops_modal_every_scroll_window_bottom_left
            self.scale = self.window.db.prefs.ui.Hops_modal_every_scroll_window_scale
    ...
    def scale_win(self):
        ...
        if self.window.db.event.mouse_dragging:
            ...
            if self.window.window_key == "Main":
                ...
                max_size = (462 * self.window.db.scale_factor, 100 * self.window.db.scale_factor)

            elif self.window.window_key == "Every_Scroll":
                max_size = (220 * self.window.db.scale_factor, 450 * self.window.db.scale_factor)
