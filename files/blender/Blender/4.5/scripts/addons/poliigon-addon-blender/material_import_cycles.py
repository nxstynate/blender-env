...
from .material_import_utils import get_node_by_type
...
if bpy.app.version >= (3, 4):
    NODE_MIX_PORT_IDX_A = 6
    NODE_MIX_PORT_IDX_B = 7
    NODE_MIX_PORT_IDX_OUT = 2
else:
    NODE_MIX_PORT_IDX_A = 1
    NODE_MIX_PORT_IDX_B = 2
    NODE_MIX_PORT_IDX_OUT = 0
...
class CyclesNodesTopLevel():
    ...
    def get_color_effective_output(self,
        ...
        if not ignore_translucency:
            ...
            idx_color_out = NODE_MIX_PORT_IDX_OUT
        ...
        if node_color is None:
            return None
        return node_color.outputs[idx_color_out]
    ...
    def get_opacity_effective_output(self) -> Optional[bpy.types.NodeSocket]:
        ...
        return node_tex_opacity.outputs[idx_alpha_port]
...
class CyclesMaterial():
    ...
    def simple_uv_group_set_defaults(self, node_group: bpy.types.Node) -> None:
        ...
        node_group.inputs["Scale"].default_value = self.params.scale
        rotation = self.params.global_rotation
        node_group.inputs["Rotation"].default_value = rotation
        translate_x = self.params.translate_x
        node_group.inputs["Translate X"].default_value = translate_x
        translate_y = self.params.translate_y
        node_group.inputs["Translate Y"].default_value = translate_y
        aspect_ratio = self.params.aspect_ratio
        node_group.inputs["Aspect Ratio"].default_value = aspect_ratio
    ...
    def create_simple_uv_group(
        ...
        node_group.node_tree.links.new(
            node_scale.inputs[0],
            node_group_inputs_internal.outputs["UV"])

        node_group.node_tree.links.new(
            node_scale.inputs[1],
            node_group_inputs_internal.outputs["Scale"])
        node_group.node_tree.links.new(
            node_aspect.inputs[1],
            node_group_inputs_internal.outputs["Aspect Ratio"])
        node_group.node_tree.links.new(
            node_translate.inputs[0],
            node_group_inputs_internal.outputs["Translate X"])
        node_group.node_tree.links.new(
            node_translate.inputs[1],
            node_group_inputs_internal.outputs["Translate Y"])
        node_group.node_tree.links.new(
            node_rotation_rad.inputs[0],
            node_group_inputs_internal.outputs["Rotation"])

        node_group.node_tree.links.new(
            node_aspect_mul.inputs[0],
            node_scale.outputs[0])
        node_group.node_tree.links.new(
            node_aspect_mul.inputs[1],
            node_aspect.outputs[0])

        node_group.node_tree.links.new(
            node_translate_add.inputs[0],
            node_aspect_mul.outputs[0])
        node_group.node_tree.links.new(
            node_translate_add.inputs[1],
            node_translate.outputs[0])

        node_group.node_tree.links.new(
            node_rotate.inputs["Vector"],
            node_translate_add.outputs[0])
        node_group.node_tree.links.new(
            node_rotate.inputs["Angle"],
            node_rotation_rad.outputs[0])

        node_group.node_tree.links.new(
            node_group_outputs_internal.inputs["UV"],
            node_rotate.outputs[0])
    ...
    def connect_color(self) -> None:
        ...
        if node_tex_ao is None:
            self.mat.node_tree.links.new(
                node_bsdf.inputs["Base Color"], node_tex_color.outputs[0])
            return
        ...
        # Here named inputs do not work, since ports are "overloaded" per
        # data types.
        self.mat.node_tree.links.new(
            node_mix.inputs[NODE_MIX_PORT_IDX_A], node_tex_color.outputs[0])
        self.mat.node_tree.links.new(
            node_mix.inputs[NODE_MIX_PORT_IDX_B], node_tex_ao.outputs[0])
        self.mat.node_tree.links.new(
            node_bsdf.inputs["Base Color"],
            node_mix.outputs[NODE_MIX_PORT_IDX_OUT])
    ...
    def connect_displacement(self) -> None:
        ...
        self.mat.node_tree.links.new(
            node_displacement.inputs["Height"],
            node_tex_displacement.outputs[0]
        )
        if self.params.mode_disp != "NORMAL":
            self.mat.node_tree.links.new(
                node_out.inputs["Displacement"], node_displacement.outputs[0])
    ...
    def connect_emission(self) -> None:
        ...
        if bpy.app.version >= (4, 0):
            if bpy.app.version >= (4, 3):
                idx_emission_col = 27
                idx_emission_strength = 28
            else:
                idx_emission_col = 26
                idx_emission_strength = 27
            node_bsdf.inputs[idx_emission_strength].default_value = 0.0
            self.mat.node_tree.links.new(
                node_bsdf.inputs[idx_emission_col], node_tex_emission.outputs[0])
        elif bpy.app.version >= (3, 0):
            node_bsdf.inputs["Emission Strength"].default_value = 0.0
            self.mat.node_tree.links.new(
                node_bsdf.inputs["Emission"], node_tex_emission.outputs[0])
        else:
            self.mat.node_tree.links.new(
                node_bsdf.inputs["Emission"], node_tex_emission.outputs[0])
            # TODO(Andreas): Is there a way to set strength in Blender < 3.0?
    ...
    def connect_fabric(self) -> None:
        ...
        if bpy.app.version >= (4, 0):
            ...
            self.mat.node_tree.links.new(
                node_bsdf.inputs[idx_sheen_tint],
                node_tex_fuzz.outputs[0])

            node_bsdf.inputs[idx_sheen_weight].default_value = 1.0  # weight
            node_bsdf.inputs[idx_sheen_rough].default_value = 0.3  # roughness
        else:
            ...
            self.mat.node_tree.links.new(
                node_mix.inputs[0], node_fresnel.outputs[0])
            self.mat.node_tree.links.new(
                node_mix.inputs[NODE_MIX_PORT_IDX_A],
                output_color)
            self.mat.node_tree.links.new(
                node_mix.inputs[NODE_MIX_PORT_IDX_B],
                node_tex_fuzz.outputs[0])
            self.mat.node_tree.links.new(
                node_bsdf.inputs["Base Color"],
                node_mix.outputs[NODE_MIX_PORT_IDX_OUT])
    ...
    def connect_metalness(self) -> None:
        ...
        node_bsdf = self.nodes.bsdf_principled
        self.mat.node_tree.links.new(
            node_bsdf.inputs["Metallic"], node_tex_metalness.outputs[0])
    ...
    def connect_normal(self) -> None:
        ...
        self.mat.node_tree.links.new(
            node_normal.inputs[1], node_tex_normal.outputs[0])
        if self.params.mode_disp == "NORMAL":
            self.mat.node_tree.links.new(
                node_bsdf.inputs["Normal"], node_normal.outputs[0])
    ...
    def connect_opacity(self) -> None:
        ...
        node_bsdf = self.nodes.bsdf_principled
        self.mat.node_tree.links.new(
            node_bsdf.inputs["Alpha"], output_opacity)
    ...
    def connect_roughness(self) -> None:
        ...
        if not is_specular:
            self.mat.node_tree.links.new(
                node_bsdf.inputs["Roughness"], node_tex_roughness.outputs[0])
        else:
            ...
            self.mat.node_tree.links.new(
                node_gloss_invert.inputs["Color"],
                node_tex_roughness.outputs[0])
            self.mat.node_tree.links.new(
                node_bsdf.inputs["Roughness"],
                node_gloss_invert.outputs[0])
    ...
    def connect_sss(self) -> None:
        ...
        if bpy.app.version >= (4, 0):
            ...
            self.mat.node_tree.links.new(
                node_bsdf.inputs["Subsurface Radius"], node_tex_sss.outputs[0])

            # Subsurface Weight
            node_bsdf.inputs[idx_sss_weight].default_value = 1.0
            # Subsurface Scale
            node_bsdf.inputs[idx_sss_scale].default_value = 0.1
            ...
            self.mat.node_tree.links.new(
                node_math.inputs[0], node_tex_sss.outputs[0])
            self.mat.node_tree.links.new(
                node_bsdf.inputs["Subsurface Radius"], node_math.outputs[0])

            node_bsdf.inputs["Subsurface"].default_value = 1.0

            output_color = self.nodes.get_color_effective_output(
                ignore_translucency=True)
            self.mat.node_tree.links.new(
                node_bsdf.inputs["Subsurface Color"],
                output_color)
    ...
    def connect_translucency(self) -> None:
        ...
        self.mat.node_tree.links.new(
            node_color_invert.inputs["Color"], node_value.outputs[0])

        self.mat.node_tree.links.new(
            node_mix_color.inputs[NODE_MIX_PORT_IDX_A],
            output_color)
        self.mat.node_tree.links.new(
            node_mix_color.inputs[NODE_MIX_PORT_IDX_B],
            node_color_invert.outputs[0])

        self.mat.node_tree.links.new(
            node_bsdf.inputs["Base Color"],
            node_mix_color.outputs[NODE_MIX_PORT_IDX_OUT])

        self.mat.node_tree.links.new(
            node_mix_translucency.inputs[NODE_MIX_PORT_IDX_A],
            node_tex_translucency.outputs[0])
        self.mat.node_tree.links.new(
            node_mix_translucency.inputs[NODE_MIX_PORT_IDX_B],
            node_value.outputs[0])

        self.mat.node_tree.links.new(
            node_translucent_bsdf.inputs["Color"],
            node_mix_translucency.outputs[NODE_MIX_PORT_IDX_OUT])
        if node_normal is not None:
            self.mat.node_tree.links.new(
                node_translucent_bsdf.inputs["Normal"],
                node_normal.outputs[0])

        self.mat.node_tree.links.new(
            node_add_shader.inputs[0], node_bsdf.outputs[0])
        self.mat.node_tree.links.new(
            node_add_shader.inputs[1], node_translucent_bsdf.outputs[0])

        if output_opacity is not None:
            self.mat.node_tree.links.new(
                node_mix_shader.inputs["Fac"],
                output_opacity)
        else:
            # Mateusz recommended to use a white value in this case
            node_mix_shader.inputs["Fac"].default_value = 1.0
        self.mat.node_tree.links.new(
            node_mix_shader.inputs[1], node_transparent_bsdf.outputs[0])
        self.mat.node_tree.links.new(
            node_mix_shader.inputs[2], node_add_shader.outputs[0])

        self.mat.node_tree.links.new(
            node_out.inputs["Surface"], node_mix_shader.outputs[0])
    ...
    def connect_transmission(self) -> None:
        ...
        self.mat.node_tree.links.new(
            node_vol_abs.inputs["Color"], node_tex_color.outputs[0])
        self.mat.node_tree.links.new(
            node_out.inputs["Volume"], node_vol_abs.outputs[0])
        ...
        if bpy.app.version >= (4, 0):
            if bpy.app.version >= (4, 3):
                ...
                idx_transmission = 17
            self.mat.node_tree.links.new(
                node_bsdf.inputs[idx_transmission], node_tex_transmission.outputs[0])
        else:
            self.mat.node_tree.links.new(
                node_bsdf.inputs["Transmission"],
                node_tex_transmission.outputs[0])
