...
class POLIIGON_OT_view_thumbnail(Operator):
    ...
    def execute(self, context):
        ...
        # TODO(Andreas):
        # cTB.reset_asset_error(asset_name=asset_name)
        ...
        try:
            ...
            # Main loading steps
            area = self._create_window(asset_name)
    ...
    def _create_window(self, asset_name: str):
        ...
        # TODO(Andreas): We seem to mis-use a built-in operator's window here.
        #                Would it be possible to extend the windows layout with
        #                buttons?
        #                Actually during 1:1 2024/07/19 there came up a
        #                different approach for popups, which could likely be
        #                used here instead as well.

        # TODO(who?): Showcase index/count once button exists to flip through.
        # asset_data = cTB._asset_index.get_asset(self.asset_id)
        # i = self.thumbnail_index if self.thumbnail_index > 0 else 1
        # count = len(asset_data.thumb_urls)
        # area.header_text_set(f"Asset thumbnail: {self.asset} ({i}/{count})")
        area.header_text_set(_t("Asset thumbnail: {0}").format(asset_name))
