...
class POLIIGON_OT_setting(Operator):
    ...
    def _set_edit_preset(self, context) -> None:
        mode_parts = self.mode.split("@")
        edit_preset = mode_parts[1]
        if cTB.vEditPreset != edit_preset:
            edit_text = mode_parts[2]
            cTB.vEditPreset = edit_preset
            context.scene.vEditText = edit_text
            return

        try:
            cTB.vPresets[cTB.vEditPreset] = [
                float(_value)
                for _value in context.scene.vEditText.replace(
                    " ", "").split(";")
            ]
    ...
    def execute(self, context):
        ...
        elif self.mode.startswith("preset@"):
            self._set_edit_preset(context)
            return {"FINISHED"}
