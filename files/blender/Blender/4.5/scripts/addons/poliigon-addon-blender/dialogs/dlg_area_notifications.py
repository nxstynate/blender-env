...
def _draw_notification_popup_message_single_row(
        cTB, notice, first_row, icon) -> bpy.types.Operator:
    # Single row with text + button.
    first_row.alert = notice.alert
    first_row.label(text=notice.title)
    first_row.alert = False
...
def _draw_notification_popup_message(
    ...
    if panel_width > single_row_width and len(notice.title) <= 80:
        op = _draw_notification_popup_message_single_row(
            cTB, notice, first_row, icon)
    else:
