...
def show_quick_menu(cTB, asset_data: AssetData) -> None:
    ...
    def draw(self, context):
        ...
        op = layout.operator(
            # TODO(Andreas): why is this operator referenced differently?
            "POLIIGON_OT_view_thumbnail",
            text=_t("View larger thumbnail"),
            icon="OUTLINER_OB_IMAGE")
        op.tooltip = _t("View larger thumbnail for {0}").format(asset_name)
        op.asset_id = asset_data.asset_id
        op.thumbnail_index = 0
