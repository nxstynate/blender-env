...
class ApiRemoteControl():
    ...
    def add_job_get_download_prefs(self,
        ...
        self.add_job(JobType.GET_DOWNLOAD_PREFS,
                     params,
                     callback_cancel,
                     callback_progress,
                     callback_done,
    ...
    def add_job(self,
        ...
        job = ApiJob(
            job_type, params, callback_cancel, callback_progress, callback_done)
        self.enqueue_job(job, force)
    ...
    def shutdown(self) -> None:
        ...
        # Prevent any new jobs of being queued
            ...
            if do_wait:
                try:
                    ...
                    self._addon._api.report_message(
                        "apirc_thread_failure", msg, "error")
            ...
            if job.job_type != JobType.EXIT:
                try:
                    ...
                    msg = ("A job's finish function failed unexpectedly: "
                           f"{str(job)}")
                    self._addon._api.report_message(
                        "apirc_job_finish_failed", msg, "error")
                    self.logger.exception(msg)
            else:
                # JobType.EXIT
                self.collect_running = False
