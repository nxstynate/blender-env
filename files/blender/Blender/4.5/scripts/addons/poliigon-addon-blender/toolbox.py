            ...
            for window in wm.windows:
                for area in window.screen.areas:
                    ...
                    for region in area.regions:
                        # TODO(Andreas): At first I thought this solved a problem on my machine.
                        #                But it didn't. Likely to change back.
                        # region.tag_redraw()
                        bpy.app.timers.register(
                            region.tag_redraw, first_interval=0)
...
class c_Toolbox(PoliigonAddon):
    ...
    def f_GetPageAssets(self, idx_page: int) -> Tuple[List[int], int]:
        ...
        idx_first_last = idx_first_asset + num_per_page
        ...
        if asset_ids is not None:
            ...
            asset_ids_page = asset_ids[idx_first_asset:idx_first_last]
    ...
    def reset_asset_error(self,
        ...
        for err in self.ui_errors:
            if asset_id and err.asset_id == asset_id:
                ...
                self.logger.debug(f"Reset error from id: {err}")
            elif asset_name and err.asset_name == asset_name:
                self.ui_errors.remove(err)
                self.logger.debug(f"Reset error from name: {err}")
...
def f_do_on_main_thread() -> float:
    ...
    if cTB.msg_download_limit is not None:
        bpy.ops.poliigon.popup_download_limit("INVOKE_DEFAULT", msg=cTB.msg_download_limit)
