...
bl_info = {
    ...
    "version": (0, 2, 3),
    "blender": (2, 80, 0),
    "location": "Item > Edit Instanced Collection",
...
class EditCollection(bpy.types.Operator):
    ...
    def execute(self, context):
        prefs = context.preferences.addons[package_name].preferences #"edit_instanced_collection"].preferences
...
class EIA_PT_Preferences(bpy.types.AddonPreferences):
    """ Preference Settings Addin Panel"""
    bl_idname = package_name #__name__
    ...
    world_texture: bpy.props.EnumProperty(name="Background",description="Background (World) texture of the temporary scene",items=[("checker", "Checker (generated map)", "Checker-like texture using a Generated map"),("checker_view", "Checker (view aligned)", "Checkerboard texture aligned to the view"),("gray", "Gray", "Solid Gray Background"),("none", "None", "No World/background (black)")])
...
def register():
    ...
    # bpy.utils.register_class(EIA_PT_Preferences)
...
def unregister():
    ...
    # bpy.utils.unregister_class(EIA_PT_Preferences)
