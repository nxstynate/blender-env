...
"""Native (Vulkan / Metal) axis-aligned box particle modes — indirect binned path."""

from __future__ import annotations

from ..native_base import NativeParticleBase

_BOX_OUTLINE_MODE_INDEX = 3
_BOX_FILLED_MODE_INDEX = 4


class Box3DNativeRenderer(NativeParticleBase):
    def draw(self, context, pipeline, scene, params) -> bool:
        ...
        bridge.stage_display_shape(
            6, 0, 1 if (rad and rad.valid) else 0, float(params.size), float(params.size)
        )
        bridge.stage_indirect_mode(_BOX_OUTLINE_MODE_INDEX)
...
class Box3DFilledNativeRenderer(NativeParticleBase):
    def draw(self, context, pipeline, scene, params) -> bool:
        bridge = self._bridge
        if not bridge.load():
            return False
        ...
        bridge.stage_display_shape(
            7, 0, 1 if (rad and rad.valid) else 0, float(params.size), float(params.size)
        )
        bridge.stage_indirect_mode(_BOX_FILLED_MODE_INDEX)
        self._stage_emitter_settings(params)
        bridge.stage_color_mode(col is not None and col.valid)
