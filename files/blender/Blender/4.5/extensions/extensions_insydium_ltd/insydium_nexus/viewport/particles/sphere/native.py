...
"""Native (Vulkan / Metal) solid UV-sphere particle mode — indirect binned path."""

from __future__ import annotations

from ..native_base import NativeParticleBase

_SPHERE_MODE_INDEX = 11


class SphereNativeRenderer(NativeParticleBase):
    def draw(self, context, pipeline, scene, params) -> bool:
        ...
        bridge.stage_display_shape(
            11, 0, 1 if (rad and rad.valid) else 0, float(params.size), float(params.size)
        )
        bridge.stage_indirect_mode(_SPHERE_MODE_INDEX)
