...
"""Blender GPU module circle filled renderer (CPU fallback)."""
...
from ..square.basic import build_camera_quad_buffers
...
def _get_shader():
    ...
    vert_out = gpu.types.GPUStageInterfaceInfo("circle_interface")
    ...
    info.fragment_source(
        ...
        "    float vig = smoothstep(1.0 - vw, 1.0, r);\n"
        "    const float k = 0.12;\n"
...
class CircleFilledBasicRenderer(ParticleRenderer):
    def draw(self, context, pipeline, scene, params) -> bool:
        ...
        batch = batch_for_shader(
            shader,
            "TRIS",
            {"pos": quad_pos, "vcol": quad_cols, "uv": quad_uvs},
        )
