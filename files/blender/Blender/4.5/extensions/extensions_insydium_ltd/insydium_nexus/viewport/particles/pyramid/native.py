...
"""Native (Vulkan / Metal) axis-aligned pyramid particle modes — indirect binned path."""

from __future__ import annotations

from ..native_base import NativeParticleBase

_PYRAMID_OUTLINE_MODE_INDEX = 7
_PYRAMID_FILLED_MODE_INDEX = 8


class PyramidNativeRenderer(NativeParticleBase):
    def draw(self, context, pipeline, scene, params) -> bool:
        ...
        bridge.stage_display_shape(
            8, 0, 1 if (rad and rad.valid) else 0, float(params.size), float(params.size)
        )
        bridge.stage_indirect_mode(_PYRAMID_OUTLINE_MODE_INDEX)
...
class PyramidFilledNativeRenderer(NativeParticleBase):
    def draw(self, context, pipeline, scene, params) -> bool:
        bridge = self._bridge
        if not bridge.load():
            return False
        ...
        bridge.stage_display_shape(
            9, 0, 1 if (rad and rad.valid) else 0, float(params.size), float(params.size)
        )
        bridge.stage_indirect_mode(_PYRAMID_FILLED_MODE_INDEX)
        self._stage_emitter_settings(params)
        bridge.stage_color_mode(col is not None and col.valid)
