    ...
    def modal(self, context, event):
        ...
        for region in regions:
            ...
            if camera_obj.perspective_plotter.ref_distance_mode  != 'camera_distance':
                ...
                length_point_a_2d, length_point_b_2d = util.get_length_points(grid_point, vp_to_project_to_px, border_width, border_height, camera_obj.perspective_plotter.length_point_a, camera_obj.perspective_plotter.length_point_b)
...
class MESH_OT_DeleteKeyFrame(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        camera_obj = util.get_camera(context, context.region)
        if (camera_obj is None) or (not camera_obj.animation_data):
            return False
...
class MESH_OT_SwitchtoCamera(bpy.types.Operator):
    """Switch to Camera View to use Perspective Plotter"""
    bl_idname = "view3d.pp_switch_to_camera"
    ...
    def execute(self, context):
        camera_obj = util.get_camera(context, context.region)
        if camera_obj:
            # Get the current area, typically a 3D view
            for area in context.screen.areas:
                if area.type == 'VIEW_3D':
                    for space in area.spaces:
                        if space.type == 'VIEW_3D':
                            ...
                            bpy.ops.view3d.perspective_plotter('INVOKE_DEFAULT')
                            break
