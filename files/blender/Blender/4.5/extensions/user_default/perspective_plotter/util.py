...
def get_length_points(grid_point, vp_to_project_to, border_width, border_height, length_point_a, length_point_b):
    '''get control points for the length measure lines'''

    # normalize the width and height by the relative frame border to handle calculations.  
    # Originally, this was the distance to the vanishing point which was not good when the vanishing point was far away.
    vec_line = (vp_to_project_to - grid_point).normalized() * (border_width + border_height)
...
def calc_view(context, camera_obj, region):
    ...
    if camera_obj.perspective_plotter.ref_distance_mode != 'camera_distance':
        ...
        point_a, point_b = get_length_points(grid_point, vp_to_project_to_px, border_width, border_height, camera_obj.perspective_plotter.length_point_a, camera_obj.perspective_plotter.length_point_b)
            ...
            for area_region in area.regions:
                if area_region.type == 'WINDOW' and \
                    ...
                    area_region.data.view_perspective == 'CAMERA' and \
                        ...
                        region_exists(context, area_region):
