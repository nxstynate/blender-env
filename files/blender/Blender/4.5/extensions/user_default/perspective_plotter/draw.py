...
def get_shader(str_type, str_type_override=None):
    '''Get a cached shader'''
    global _shader_map
    
    # Check if the shader for str_type is already cached
    if str_type in _shader_map:
        return _shader_map[str_type]
    
    # If str_type_override is already cached, return it
    if str_type_override and str_type_override in _shader_map:
        return _shader_map[str_type_override]
    
    try:
        # Try to fetch and cache the shader for str_type
        _shader_map[str_type] = gpu.shader.from_builtin(str_type)
        return _shader_map[str_type]
    except ValueError:
        # If there's an exception and str_type_override is provided, try that
        if str_type_override:
            _shader_map[str_type_override] = gpu.shader.from_builtin(str_type_override)
            return _shader_map[str_type_override]
        
    # If everything fails, raise an error or return a default value
    raise ValueError("Unable to fetch shader")
...
def draw_line_3d(color, start, end, width=1):
    ...
    vertices = [start, end]
    shader = get_shader('UNIFORM_COLOR', '3D_UNIFORM_COLOR')
...
def draw_axis_line_2d(color, start, end, line_width=1, circle_diameter=10):
    ...
    shader = get_shader('UNIFORM_COLOR', '2D_UNIFORM_COLOR')
